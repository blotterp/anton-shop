    ...
    if ($product->product_type === 'variable') {
        if ($price_type === 'price' && !$sale) {
            ...
            if ($product->get_variation_sale_price('min', true) >= $price) {
                $price = $product->get_variation_sale_price('max', true);
            }
        }
    } else {
        if ($price_type === 'price' && !$sale) {
...
function ajaxs_load_product($jx)
{
    ...
    $jx->jseval("Webflow.destroy(); Webflow.ready(); if(Webflow.require('ix2') != undefined) { Webflow.require('ix2').init(); }");
}
...
function ajaxs_add_to_cart($jx)
{
    ...
    $jx->jseval("Webflow.destroy(); Webflow.ready(); if(Webflow.require('ix2') != undefined) {
